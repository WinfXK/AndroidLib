...
import cn.winfxk.android.androidlib.materialishprogress.ProgressWheel;
...
public class ProgressHelper {
    ...
    public ProgressHelper(Context ctx) {
        mToSpin = true;
        mSpinSpeed = 0.75f;
        mBarWidth = ctx.getResources().getDimensionPixelSize(libR.dimen.common_circle_width) + 1;
        mBarColor = ctx.getResources().getColor(libR.color.success_stroke_color, ctx.getTheme());
        mRimWidth = 0;
        mRimColor = 0x00000000;
        mIsInstantProgress = false;
        mProgressVal = -1;
        mCircleRadius = ctx.getResources().getDimensionPixelOffset(libR.dimen.progress_circle_radius);
    }
